<?php
    ...
    function dbg_echo_tab($tab) {  // fonction utilisée pour l'affichage en mode DEBUG
        global $DEBUG;  // on récupère la variable $DEBUG qui est extérieure au scope de la fonction
        if (!$DEBUG) return;
        echo "<pre>";
        print_r($tab);
    ...
    for ($i=0; $i<$infosLab["nbTiles"]; $i++) {
        $labyrinthe[]=$objTile;
        $labyrinthe[$i]["valeur"] = $i;

        if ($i < $infosLab["width"]) {
            $labyrinthe[$i]["murN"]=0;
        }
        if($i > ($infosLab["nbTiles"]-1)-$infosLab["width"]){
            $labyrinthe[$i]["murS"]=0;
        }
        if($i%$infosLab["width"]==0){
            $labyrinthe[$i]["murO"]=0;
        }
        if($i%$infosLab["width"]==$infosLab["width"]-1){
            $labyrinthe[$i]["murE"]=0;
        }
    }
    ...
    function choisirTileAdjacenteAlea($tile) {
        /**
         * renvoie l'indice d'une case adjacente à celle passée en paramètre avec un numero
         */
    ...
    while($infosLab["nbOpenWalls"] != $infosLab["nbOpenWallsTarget"]) {
        $indice = rand(0, $infosLab["nbTiles"]);
